...
class LogConfigReader():
    ...
    def __init__(self):
        ...
        if (not os.path.isdir(roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/log")):
            logger.info("No user config found, copying dist files")
            os.makedirs(roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/log")
            for f in glob.glob(roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/cfclient/configs/log/[A-Za-z]*.json"):
                shutil.copy2(f, roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/log")

    def readConfigFiles(self):
        """Read and parse log configurations"""
        configsfound = [ os.path.basename(f) for f in glob.glob(roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/log/[A-Za-z_-]*.json")]

        for conf in configsfound:            
            try:
                logger.info("Parsing [%s]", conf)
                json_data = open (roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/log/%s"%conf)
    ...
    def saveLogConfigFile(self, logconfig):
        ...
        filename = roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib" + "/log/" + logconfig.getName() + ".json"

...
class ROSNode():
    ...
    def get_data(self, data, inputMap):   
    
        if self.buttons == None or self.axes == None:
            return data                
        ...
        for key, value in inputMap.iteritems():
            if key[6]=="A":               
                ...
                if value["name"] == "yawRight":
                    yaw = yaw + (1-self.axes[value["id"]])  * value["scale"]
                elif value["name"] == "yawLeft":
                    yaw = yaw + (1-self.axes[value["id"]])  * value["scale"]
                elif value["name"] == "yaw":                    
                    yaw = self.axes[value["id"]]  * value["scale"]
                else:
                    data[value["key"]] = self.axes[value["id"]]  * value["scale"]    
                   
                    
            if key[6]=="B":                               
                val = self.buttons[value["id"]]                 
                if val:
                    name = value["key"]
                    if name == "estop":                    
                        data["estop"] = not self.estop
                        self.estop = not self.estop
                    elif name == "exit":
                        data["exit"] = True
                    else: # Generic cal for pitch/roll
                        data[name] = value["scale"]                                   
    ...
    def new_joydata(self, joymsg): 
        self.axes = joymsg.axes
        self.buttons = joymsg.buttons  
...
class PyGameReader():
    ...
    def startInput(self, deviceId, inputMap):
        self._use_ros = deviceId == self._device_count
        if self._use_ros:
            print "Using ROS node as input"
    ...
    def readInput(self):
        ...
        if self._use_ros:
            self.data = self.j.get_data(self.data, self.inputMap)
        else:
    ...
    def getAvailableDevices(self):
        ...
        dev.append({"id": nbrOfInputs, "name" : "ROSJoy"})
        return dev

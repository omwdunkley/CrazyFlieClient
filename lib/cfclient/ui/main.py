...
path = roslib.packages.get_pkg_dir('crazyflie_ros') + "/lib/cfclient/ui/main.ui"
main_window_class = uic.loadUiType(path)[0]
...
class MainUI(QtGui.QMainWindow, main_window_class):
    ...
    def __init__(self, *args):
        ...
        self.cfg = Config()
        self.cf = Crazyflie(ro_cache=roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib/cflib/cache",
                            rw_cache=roslib.packages.get_pkg_dir('crazyflie_ros')+"/lib/cache")
